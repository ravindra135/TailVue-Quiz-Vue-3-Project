<script>
  export default {
    ...
    methods: {
      submitAnswer(answer) {
        const isCorrectAnswer = (answer === this.correct_answer);
...
<template>
  ...
  <div class="grid grid-cols-2 gap-4 mb-8">
      <div v-for="(option, index) in options" 
          ...
          class="border-2 border-gray-300 p-5 rounded-xl cursor-pointer font-noto-sans text-xl font-semibold hover:bg-blue-400 hover:text-white hover:ring-2 hover:ring-blue-300 transition duration-100 ease-in text-center"
          v-html="option">
      </div>
  </div>

  <div class="flex items-center justify-between">
    <p class="text-gray-400 text-sm font-noto-sans">Category: <span v-html="category"></span></p>
